...
def read_and_decode(filename, w, h, one_hot=True, n_class=None,
                    is_train=None, bResize=False, origImgW=0, origImgH=0):
...
def compile_tfrecord(train_model, optimizer, loss, out_tensor_lst, is_sequential=False, metrics=[], loss_weights=None):
    ...
    K.set_value(train_model.optimizer.lr, 1.0 * lr)
        ...
        for metric in output_metrics:
            if metric == 'accuracy' or metric == 'acc':
                ...
                if (output_shape[-1] == 1) or \
                   (train_model.loss_functions[i] == objectives.binary_crossentropy):
...
def fit_tfrecord(train_model, nb_train_sample, batch_size, nb_epoch=10, verbose=1, callbacks=[],
    ...
    def _make_train_function(model):
        ...
        if model.train_function is None:
            ...
            if platformType == 'Windows':
                training_updates = model.optimizer.get_updates(params = model._collected_trainable_weights,
    ...
    for epoch in range(initial_epoch, nb_epoch):
        ...
        epoch_logs = {}
        batches_of_epoch = nb_train_sample//batch_size
        for batch_index in range(0, batches_of_epoch):
            batch_logs = {}
            ...
            for l, o in zip(out_labels, outs):
                if l == 'acc' or l == 'mean_absolute_error':
                    total_acc = 0.
                    for i in range(len(o)):
                        total_acc += o[i]
                    total_acc = total_acc / len(o)
                    batch_logs[l] = total_acc
                elif l == 'loss':
                    batch_logs[l] = o
                else:
                    pass
